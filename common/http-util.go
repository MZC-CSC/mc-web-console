...
func init() {
	...
	if err := viper.ReadInConfig(); err != nil {
		panic(fmt.Errorf("fatal error reading conf/api.yaml file: %s", err))
	}
...
// AnyCaller는 echo.Context, operationId, commonRequest, auth유무 를 받아 conf/api.yaml 정보를 바탕으로 commonCaller를 호출합니다.
...
// SubsystemAnyCaller echo.Context, subsystemName, operationId, commonRequest, auth유무 를 받아 conf/api.yaml 정보를 바탕으로 commonCaller를 호출합니다.
