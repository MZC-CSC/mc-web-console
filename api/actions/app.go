...
func App() *buffalo.App {
	appOnce.Do(func() {
		...
		if MCIAM_USE { // MCIAM USE True
			...
			// API Management routes (개발자 전용)
			apiMgmt := app.Group(apiPath + "/v1/apimanagement")
			apiMgmt.GET("/frameworks", ListFrameworks)
			apiMgmt.GET("/frameworks/{frameworkName}", GetFramework)
			apiMgmt.POST("/frameworks", CreateFramework)
			apiMgmt.PUT("/frameworks/{frameworkName}", UpdateFramework)
			apiMgmt.DELETE("/frameworks/{frameworkName}", DeleteFramework)
			apiMgmt.GET("/frameworks/{frameworkName}/versions", ListFrameworkVersions)
			apiMgmt.POST("/frameworks/{frameworkName}/versions", AddFrameworkVersion)
			apiMgmt.PUT("/frameworks/{frameworkName}/versions/{versionNumber}", UpdateFrameworkVersion)
			apiMgmt.DELETE("/frameworks/{frameworkName}/versions/{versionNumber}", DeleteFrameworkVersion)
			apiMgmt.PUT("/frameworks/{frameworkName}/active-version", SetActiveVersion)
			apiMgmt.POST("/frameworks/{frameworkName}/versions/{versionNumber}/sync", SyncFrameworkVersion)
			apiMgmt.POST("/frameworks/sync-all", SyncAllFrameworks)
			apiMgmt.GET("/operations", GetApiOperations)
			apiMgmt.GET("/active-versions", GetActiveVersions)
